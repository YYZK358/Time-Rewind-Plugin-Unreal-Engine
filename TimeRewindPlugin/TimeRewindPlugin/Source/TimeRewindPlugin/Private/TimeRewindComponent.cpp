...
void UTimeRewindComponent::BeginPlay()
{
	Super::BeginPlay();
	Owner = GetOwner();
	AddToTimeActorManagerSubsystem(Owner);
...
void UTimeRewindComponent::AddToTimeActorManagerSubsystem(AActor* Actor)
{
	UTimeActorManagementSubsystem* manager = GetWorld()->GetSubsystem<UTimeActorManagementSubsystem>();
	if (manager)
	{
